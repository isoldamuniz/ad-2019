...
export default function App() {
  ...
  const realizarSorteio = async () => {
    ...
    const pessoasShuffled = shuffle(pessoas);

    const rascunho = pessoasShuffled.map((pessoa, index) => {
          ...
          <tbody>
            <tr>
              ...
              <td style={{ textAlign: "right" }}>
                <InsertButton click={handleOpenForm} />
              </td>
              <td style={{ textAlign: "right" }}>
                <SorteioButton click={realizarSorteio} />
              </td>
            </tr>
            <tr>
              <td colSpan="3">
                ...
                {showFormFlag && (
                  <InsertForm onSave={insertPessoa} onClose={handleFormClose} />
                )}
                {showFormSorteioFlag && (
                  <SorteioForm
                    onClose={handleFormSorteioClose}
                    submitMsg={submitMsg}
                    sorteio={sorteio}
                  />
                )}
                <Grid
                  fluid={true}
                  toEdit={handleOpenEditForm}
                  toDelete={deletePessoa}
                  pessoas={pessoas.length !== 0 ? pessoas : []}
                />
              </td>
            </tr>
          </tbody>
